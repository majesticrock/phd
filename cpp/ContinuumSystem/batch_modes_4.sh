#!/bin/bash
rm -rf auto_generated_4/
mkdir -p auto_generated_4
...
for NEW_VALUE in "${NEW_VALUES[@]}"; do
  ...
  while read line; do
    ...
    if [[ "$TOKEN_NAME" == "$TOKEN" ]]; then
      ...
      sed "s/$TOKEN_NAME $TOKEN_VALUE/$TOKEN_NAME $NEW_VALUE/" params/cluster.config > auto_generated_4/$NEW_NAME.config
      break
    fi
  ...
  # Execute the program
  sbatch auto_generated_4/$NEW_NAME.slurm
done
