...
for NEW_VALUE in "${NEW_VALUES[@]}"; do
  ...
  sed -i "s|#SBATCH --job-name=modes|#SBATCH --job-name=ul$NEW_NAME|" auto_generated_ul_3/$NEW_NAME.slurm
  sed -i "s|#SBATCH --output=/home/althueser/phd/cpp/ContinuumSystem/modes_output.txt|#SBATCH --output=/home/althueser/phd/cpp/ContinuumSystem/output_ul_$NEW_NAME.txt|" auto_generated_ul_3/$NEW_NAME.slurm
  sed -i "s|mpirun ./build_cluster/ContinuumSystem params/ul_cluster.config|mpirun ./build_cluster/ContinuumSystem auto_generated_ul_3/$NEW_NAME.config|" auto_generated_ul_3/$NEW_NAME.slurm
