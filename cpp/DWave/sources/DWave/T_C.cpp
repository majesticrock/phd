...
T_C::T_C(mrock::utility::InputFileReader& input) : model(Model(input)) {
...
void T_C::compute() {
    ...
    auto increase_dT = [&]() -> bool {
        if (current_dT >= 0.5 * INITIAL_DT)
            return false;
        if (is_zero(last_delta))
            return false;
        if (std::abs((delta_max - last_delta) / last_delta) > 0.05)
            return false;
    ...
    do {
        ...
        const auto found_iter =
            std::find_if(temperatures.begin(), temperatures.end(),
                         [&T](const l_float Tvec) -> bool { return std::abs(Tvec - T) < TARGET_DT; });
        if (found_iter != temperatures.end()) {
            const std::size_t index = std::distance(temperatures.begin(), found_iter);

            last_delta = std::abs(std::ranges::max(
                finite_gaps[index], [](const double lhs, const double rhs) { return std::abs(lhs) < std::abs(rhs); }));
        ...
        if (!model.Delta.converged) {
            std::cerr << "Self-consistency not achieved while computing T_C! Retrying... at beta=" << model.beta
                      << std::endl;
            solver.compute(false, BROYDEN_ITER, BROYDEN_EPS);
            index_at_ef = static_cast<int>(0.5 * model.N * (model.chemical_potential + 1));
            delta_max = model.delta_max();
        ...
        std::cout << "\t\tDelta_max=" << delta_max << "\tDelta_F=" << model.Delta[index_at_ef]
        ...
        if (decrease_dT()) {
            ...
            T -= current_dT;
            if (T < 0)
                T = 0.0;  // circumvent rare case floating point arithmetic issues
            current_dT *= 0.2;
            // As we say in German: man muss ja nicht gleich uebertreiben...
            if (current_dT < 0.5 * TARGET_DT)
                current_dT = 0.5 * TARGET_DT;
        } else {
            if (increase_dT()) {
                current_dT *= 2.0;
            }
    ...
    std::sort(indices.begin(), indices.end(),
              [&](std::size_t A, std::size_t B) -> bool { return temperatures[A] < temperatures[B]; });
    ...
    for (std::size_t i = 0U; i < delta_maxs.size(); ++i) {
        delta_maxs[i] = std::abs(std::ranges::max(
            finite_gaps[i], [](const l_float A, const l_float B) -> bool { return std::abs(A) < std::abs(B); }));
    }

    std::cout << "Finished T_C computation at T_C = " << T << " (beta=" << 1. / (T > 0.0 ? T : -1.) << ") in "
              << temperatures.size() << "iterations." << std::endl;
}
}  // namespace DWave
