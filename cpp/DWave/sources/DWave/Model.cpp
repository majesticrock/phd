...
Model::Model(mrock::utility::InputFileReader& input)
    ...
    for (int kx = 0; kx < N; ++kx) {
        for (int ky = 0; ky < N; ++ky) {
            // std::cout << kx << "\t" << ky << "\t" << dispersion(kx, ky) << "\t" << fermi_function(dispersion(kx, ky),
            // -1) << std::endl;
...
void Model::iteration_step(const ParameterVector& initial_values, ParameterVector& result) {
    ...
    auto fit_occupation = [&](const l_float mu) -> l_float { return filling_at_zero_temp - compute_filling(mu); };
    ...
    const auto best_mu = boost::math::tools::toms748_solve(
        fit_occupation, bracket.first, bracket.second, boost::math::tools::eps_tolerance<l_float>(32), boost_max_it);

    const l_float fa{fit_occupation(best_mu.first)};
    const l_float fb{fit_occupation(best_mu.second)};
    result(N * N) = is_zero(fb - fa) ? 0.5 * (best_mu.first + best_mu.second)
                                     : (best_mu.first * fb - best_mu.second * fa) / (fb - fa);
...
#pragma omp parallel for
    for (int k = 0; k < N * N; k++) {
        ...
        for (int l = 0; l < N * N; ++l) {
            __part -=
                _expecs[mrock::symbolic_operators::OperatorType::SC][l] * dwave_factor(unravel_x(l), unravel_y(l));
        }
...
l_float Model::sc_expectation_value(int kx, int ky) const {
    ...
    if (beta < 0)
        ...
        return -Delta[ravel_index(kx, ky)] / (2 * quasiparticle_dispersion(kx, ky)) *
               std::tanh(0.5 * beta * quasiparticle_dispersion(kx, ky));
}
...
l_float Model::compute_filling(const l_float mu) const {
    auto number_operator = [&](int kx, int ky) -> l_float {
        ...
        const l_float E = std::sqrt(eps * eps + Delta[ravel_index(kx, ky)] * Delta[ravel_index(kx, ky)]);
        if (is_zero(Delta[ravel_index(kx, ky)]))
...
#pragma omp parallel for
    for (int k = 0; k < N * N; ++k) {
        _expecs.at(mrock::symbolic_operators::OperatorType::Number)[k] =
            this->occupation_number(unravel_x(k), unravel_y(k));
        _expecs.at(mrock::symbolic_operators::OperatorType::SC)[k] =
            this->sc_expectation_value(unravel_x(k), unravel_y(k));
    }
...
l_float Model::delta_max() const {
    return std::abs(*std::max_element(Delta.begin(), Delta.begin() + N * N, [](const l_float& lhs, const l_float& rhs) {
...
std::string Model::to_folder() const {
    ...
    return "/N=" + std::to_string(N) + "/" + "V=" + improved_string(dwave_coupling_in) + "/" +
           "E_F=" + improved_string(fermi_energy) + "/" + "g=" + improved_string(phonon_coupling_in) + "/" +
