...
#include <dwave/info.h>

#ifndef OUTPUT_DATA_DIR
#define OUTPUT_DATA_DIR "../../data/dwave/"
#endif
...
int main(int argc, char** argv) {
    ...
    if (true) {
        ...
        const std::string output_folder =
            std::string(OUTPUT_DATA_DIR) + input.getString("output_folder") + "/" + model.to_folder();
        ...
        nlohmann::json jDelta = {{"Delta", model.Delta.as_vector(model.N * model.N)},
                                 {"Delta_max", model.delta_max()},
                                 {"Delta_true", model.delta_true()},
                                 {"chemical_potential", model.chemical_potential}};

        jDelta.merge_patch(comments);
        mrock::utility::save_string(jDelta.dump(4), output_folder + "single_gap.json.gz");
    ...
    const std::string output_folder =
        std::string(OUTPUT_DATA_DIR) + input.getString("output_folder") + "/" + tc.model.to_folder();
    std::filesystem::create_directories(output_folder);
    std::cout << "Saving data to " << output_folder << std::endl;
    mrock::utility::save_string(info_json.dump(4), output_folder + "metadata.json.gz");

    nlohmann::json jT_C = {{"temperatures", tc.temperatures},
    ...
    mrock::utility::save_string(jT_C.dump(4), output_folder + "T_C.json.gz");

    jAllGaps.merge_patch(comments);
    mrock::utility::save_string(jAllGaps.dump(4), output_folder + "all_gaps.json.gz");
