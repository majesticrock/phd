...
for NEW_VALUE in "${NEW_VALUES[@]}"; do
  ...
  while read line; do
    ...
    if [[ "$TOKEN_NAME" == "$TOKEN" ]]; then
      ...
      sed -i "s/$TOKEN_NAME $TOKEN_VALUE/$TOKEN_NAME $NEW_VALUE/" params/params_auto_3.config
      break
    fi
  done < params/params_auto_3.config
  
  # Execute the program
  mpirun -n $n_mpi --map-by node:PE=$n_omp --bind-to core ./build/main params/params_auto_3.config 2> >(while read line; do echo -e "${RED}$line${NC}"; done)
  wait
done