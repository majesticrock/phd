...
namespace Hubbard::DensityOfStates {
	...
	public:
		...
		inline FinalType compute_weight(int k) const {
			return (_step * half_pi<FinalType>() * std::cosh(k * _step))
				/ static_cast<FinalType>(pow(cosh(half_pi<RealType>() * _sinh_x), 2));
		};
		...
		FinalType initial_filling(const UnaryFunction& dos, SaveTo& save_to) {
			...
			auto fill_vectors = [&](int k, bool sign) {
				do {
					...
					integral += save_to.values->back() * save_to.weights->back();
				} while (std::abs(save_to.values->back() * save_to.weights->back()) > 1e-11
					|| std::abs(save_to.weights->back()) > 1e-8);
				return k - 1;
			};
