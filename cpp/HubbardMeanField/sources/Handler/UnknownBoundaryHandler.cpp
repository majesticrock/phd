...
void UnknownBoundaryHandler::execute(Utility::InputFileReader& input) const {
	...
	for (int i = 0; i < FIRST_IT_STEPS; ++i) {
		...
		if (result_lower == result_upper) {
			...
			std::cout << "Rank #" << rank << ": skipped iteration #" << i << " with runtime " << std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count() << "[ms]";
			std::cout << " - both are " << (result_lower ? "negative" : "positive") << " - " << modelParameters << std::endl;
