The Coulomb interaction term is
\begin{align*}
	H_\mathrm{C} = 	&+(1/2) \cdot \sum_{ k l q } \sum_{ \sigma \sigma' } V( k, l, q )   c_{ k, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&+(1/2) \cdot \sum_{ k l q } \sum_{ \sigma \sigma' } V( l, k, q )   c_{ k, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma } 
,
\end{align*}
Here, we used the symmetry of the potential $V(k, l, q) = V(l, k, q)$ and $V(k, l, q) = V(k, l, -q)$.We can even define
\begin{equation}
	V^\mathrm{sym} (k, l, q) \equiv (1/2) \cdot (V(k, l, q) + V(l, k, -q))
\end{equation}
because this quantity will appear throughout the flow.
Our generator for the CUT is 
\begin{align*}
	\eta = 	&+1 \cdot \sum_{ P Q } \sum_{ S } A( P, Q )   b_{ -Q, }^\dagger  c_{ P+Q, S }^\dagger  c_{ P, S }  \\
	&-1 \cdot \sum_{ P Q } \sum_{ S } B( P, Q )   b_{ Q, } c_{ P+Q, S }^\dagger  c_{ P, S } 
, \end{align*}
where
\begin{align}
	A(P,Q) &= M_{P,Q} \sgn ( \epsilon_{P+Q} - \epsilon_P + \omega_{-Q} ) \\
	B(P,Q) &= M_{P+Q,-Q}^* \sgn ( \epsilon_{P+Q} - \epsilon_P - \omega_Q )
\end{align}

The first commutation, while omitting terms with 6 fermionic operators, yields
\begin{align*}
	[\eta, H_\mathrm{C}] = 	&+(1/2) \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } A( k, p )  V( k, l, q )   b_{ -p, }^\dagger  c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&+(1/2) \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } A( l-p, p )  V( k+p, l-p, -p+q )   b_{ -p, }^\dagger  c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&-(1/2) \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } A( k+q, p )  V( k+p, l, q )   b_{ -p, }^\dagger  c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&-(1/2) \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } A( l-q, p )  V( k+p, l, -p+q )   b_{ -p, }^\dagger  c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&+(1/2) \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } A( k, p )  V( l, k, q )   b_{ -p, }^\dagger  c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&+(1/2) \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } A( l-p, p )  V( l-p, k+p, -p+q )   b_{ -p, }^\dagger  c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&-(1/2) \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } A( k+q, p )  V( l, k+p, q )   b_{ -p, }^\dagger  c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&-(1/2) \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } A( l-q, p )  V( l, k+p, -p+q )   b_{ -p, }^\dagger  c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&-(1/2) \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } B( k, p )  V( k, l, q )   b_{ p, } c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&-(1/2) \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } B( l-p, p )  V( k+p, l-p, -p+q )   b_{ p, } c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&+(1/2) \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } B( k+q, p )  V( k+p, l, q )   b_{ p, } c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&+(1/2) \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } B( l-q, p )  V( k+p, l, -p+q )   b_{ p, } c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&-(1/2) \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } B( k, p )  V( l, k, q )   b_{ p, } c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&-(1/2) \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } B( l-p, p )  V( l-p, k+p, -p+q )   b_{ p, } c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&+(1/2) \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } B( k+q, p )  V( l, k+p, q )   b_{ p, } c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&+(1/2) \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } B( l-q, p )  V( l, k+p, -p+q )   b_{ p, } c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma } 
. \end{align*}
Here, we recognize our $V^\mathrm{sym}$! Symplifying the expression to
\begin{align*}
	[\eta, H_\mathrm{C}] = 	&+1 \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } A( k, p )  V^\mathrm{sym}( k, l, q )   b_{ -p, }^\dagger  c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&+1 \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } A( l-p, p )  V^\mathrm{sym}( k+p, l-p, -p+q )   b_{ -p, }^\dagger  c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&-1 \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } A( k+q, p )  V^\mathrm{sym}( k+p, l, q )   b_{ -p, }^\dagger  c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&-1 \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } A( l-q, p )  V^\mathrm{sym}( k+p, l, -p+q )   b_{ -p, }^\dagger  c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&-1 \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } B( k, p )  V^\mathrm{sym}( k, l, q )   b_{ p, } c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&-1 \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } B( l-p, p )  V^\mathrm{sym}( k+p, l-p, -p+q )   b_{ p, } c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&+1 \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } B( k+q, p )  V^\mathrm{sym}( k+p, l, q )   b_{ p, } c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&+1 \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } B( l-q, p )  V^\mathrm{sym}( k+p, l, -p+q )   b_{ p, } c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma } 
, \end{align*}
which has no two-electron interaction terms. Thus, we include the resulting terms into our Hamiltonian as 
\begin{align*}
	H'= 	&+1 \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } C_1( k, l, p, q )   b_{ -p, }^\dagger  c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&+1 \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } C_2( k, l, p, q )   b_{ p, } c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma } 
, \end{align*}
We commute again with $H'$ and obtain
\begin{align*}
	[\eta, H'] = 	&+1 \cdot \sum_{ p l k q Q } \sum_{ \sigma \sigma' } A( l+q, p )  C_1( l, k, q, Q )   b_{ -q, }^\dagger  b_{ -p, }^\dagger  c_{ l+p+q, \sigma }^\dagger  c_{ k, \sigma' }^\dagger  c_{ -Q+k, \sigma' } c_{ Q+l, \sigma }  \\
	&+1 \cdot \sum_{ p l k q Q } \sum_{ \sigma \sigma' } A( k, p )  C_1( l, k, q, Q )   b_{ -q, }^\dagger  b_{ -p, }^\dagger  c_{ l+q, \sigma }^\dagger  c_{ k+p, \sigma' }^\dagger  c_{ -Q+k, \sigma' } c_{ Q+l, \sigma }  \\
	&-1 \cdot \sum_{ p l k q Q } \sum_{ \sigma \sigma' } A( Q+l-p, p )  C_1( l, k, q, Q )   b_{ -q, }^\dagger  b_{ -p, }^\dagger  c_{ l+q, \sigma }^\dagger  c_{ k, \sigma' }^\dagger  c_{ -Q+k, \sigma' } c_{ Q+l-p, \sigma }  \\
	&-1 \cdot \sum_{ p l k q Q } \sum_{ \sigma \sigma' } A( -Q+k-p, p )  C_1( l, k, q, Q )   b_{ -q, }^\dagger  b_{ -p, }^\dagger  c_{ l+q, \sigma }^\dagger  c_{ k, \sigma' }^\dagger  c_{ -Q+k-p, \sigma' } c_{ Q+l, \sigma }  \\
	&+1 \cdot \sum_{ p l k q Q } \sum_{ \sigma \sigma' } A( l+q, p )  C_2( l, k, q, Q )   b_{ -p, }^\dagger  b_{ q, } c_{ l+p+q, \sigma }^\dagger  c_{ k, \sigma' }^\dagger  c_{ -Q+k, \sigma' } c_{ Q+l, \sigma }  \\
	&+1 \cdot \sum_{ p l k q Q } \sum_{ \sigma \sigma' } A( k, p )  C_2( l, k, q, Q )   b_{ -p, }^\dagger  b_{ q, } c_{ l+q, \sigma }^\dagger  c_{ k+p, \sigma' }^\dagger  c_{ -Q+k, \sigma' } c_{ Q+l, \sigma }  \\
	&-1 \cdot \sum_{ p l k q Q } \sum_{ S \sigma \sigma' } A( p, -q )  C_2( l, k, q, Q )   c_{ l+q, \sigma }^\dagger  c_{ k, \sigma' }^\dagger  c_{ p-q, S }^\dagger  c_{ p, S } c_{ -Q+k, \sigma' } c_{ Q+l, \sigma }  \\
	&-1 \cdot \sum_{ p l k q Q } \sum_{ \sigma \sigma' } A( Q+l-p, p )  C_2( l, k, q, Q )   b_{ -p, }^\dagger  b_{ q, } c_{ l+q, \sigma }^\dagger  c_{ k, \sigma' }^\dagger  c_{ -Q+k, \sigma' } c_{ Q+l-p, \sigma }  \\
	&-1 \cdot \sum_{ p l k q Q } \sum_{ \sigma \sigma' } A( -Q+k-p, p )  C_2( l, k, q, Q )   b_{ -p, }^\dagger  b_{ q, } c_{ l+q, \sigma }^\dagger  c_{ k, \sigma' }^\dagger  c_{ -Q+k-p, \sigma' } c_{ Q+l, \sigma }  \\
	&-1 \cdot \sum_{ p l k q } \sum_{ \sigma \sigma' } A( k+p+q, -k )  C_2( p, l, k, q )   c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+p+q, \sigma }  \\
	&-1 \cdot \sum_{ p l k q } \sum_{ \sigma \sigma' } A( k+l-q, -k )  C_2( p, l, k, q )   c_{ k+p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ k+l-q, \sigma' } c_{ p+q, \sigma }  \\
	&-1 \cdot \sum_{ p l k q Q } \sum_{ S \sigma \sigma' } B( p, -q )  C_1( l, k, q, Q )   c_{ l+q, \sigma }^\dagger  c_{ k, \sigma' }^\dagger  c_{ p-q, S }^\dagger  c_{ p, S } c_{ -Q+k, \sigma' } c_{ Q+l, \sigma }  \\
	&-1 \cdot \sum_{ p l k q Q } \sum_{ \sigma \sigma' } B( l+q, p )  C_1( l, k, q, Q )   b_{ -q, }^\dagger  b_{ p, } c_{ l+p+q, \sigma }^\dagger  c_{ k, \sigma' }^\dagger  c_{ -Q+k, \sigma' } c_{ Q+l, \sigma }  \\
	&-1 \cdot \sum_{ p l k q Q } \sum_{ \sigma \sigma' } B( k, p )  C_1( l, k, q, Q )   b_{ -q, }^\dagger  b_{ p, } c_{ l+q, \sigma }^\dagger  c_{ k+p, \sigma' }^\dagger  c_{ -Q+k, \sigma' } c_{ Q+l, \sigma }  \\
	&+1 \cdot \sum_{ p l k q Q } \sum_{ \sigma \sigma' } B( Q+l-p, p )  C_1( l, k, q, Q )   b_{ -q, }^\dagger  b_{ p, } c_{ l+q, \sigma }^\dagger  c_{ k, \sigma' }^\dagger  c_{ -Q+k, \sigma' } c_{ Q+l-p, \sigma }  \\
	&+1 \cdot \sum_{ p l k q Q } \sum_{ \sigma \sigma' } B( -Q+k-p, p )  C_1( l, k, q, Q )   b_{ -q, }^\dagger  b_{ p, } c_{ l+q, \sigma }^\dagger  c_{ k, \sigma' }^\dagger  c_{ -Q+k-p, \sigma' } c_{ Q+l, \sigma }  \\
	&-1 \cdot \sum_{ p l k q } \sum_{ \sigma \sigma' } B( k+p, -k )  C_1( p, l, k, q )   c_{ p, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ p+q, \sigma }  \\
	&-1 \cdot \sum_{ p l k q } \sum_{ \sigma \sigma' } B( l, -k )  C_1( p, l, k, q )   c_{ k+p, \sigma }^\dagger  c_{ -k+l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ p+q, \sigma }  \\
	&-1 \cdot \sum_{ p l k q Q } \sum_{ \sigma \sigma' } B( l+q, p )  C_2( l, k, q, Q )   b_{ q, } b_{ p, } c_{ l+p+q, \sigma }^\dagger  c_{ k, \sigma' }^\dagger  c_{ -Q+k, \sigma' } c_{ Q+l, \sigma }  \\
	&-1 \cdot \sum_{ p l k q Q } \sum_{ \sigma \sigma' } B( k, p )  C_2( l, k, q, Q )   b_{ q, } b_{ p, } c_{ l+q, \sigma }^\dagger  c_{ k+p, \sigma' }^\dagger  c_{ -Q+k, \sigma' } c_{ Q+l, \sigma }  \\
	&+1 \cdot \sum_{ p l k q Q } \sum_{ \sigma \sigma' } B( Q+l-p, p )  C_2( l, k, q, Q )   b_{ q, } b_{ p, } c_{ l+q, \sigma }^\dagger  c_{ k, \sigma' }^\dagger  c_{ -Q+k, \sigma' } c_{ Q+l-p, \sigma }  \\
	&+1 \cdot \sum_{ p l k q Q } \sum_{ \sigma \sigma' } B( -Q+k-p, p )  C_2( l, k, q, Q )   b_{ q, } b_{ p, } c_{ l+q, \sigma }^\dagger  c_{ k, \sigma' }^\dagger  c_{ -Q+k-p, \sigma' } c_{ Q+l, \sigma } 
\end{align*}
Here, we want to omit all terms with bosonic creation/annihilation operators, claiming them to be small. We proceed in the same manner with hexatic terms.
It is also nice to directly sort the summations so that the operators appear as they do in the Coulomb interaction. This yields
\begin{align*}
	[\eta, H'] \approx 	&-1 \cdot \sum_{ p l k q } \sum_{ \sigma \sigma' } A( k+q, p )  C_2( k+p, l, -p, q )   c_{ k, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&-1 \cdot \sum_{ p l k q } \sum_{ \sigma \sigma' } A( l-q, p )  C_2( k+p, l, -p, -p+q )   c_{ k, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&-1 \cdot \sum_{ k l p q } \sum_{ \sigma \sigma' } B( k+p, -p )  C_1( k, l, p, q )   c_{ k, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma }  \\
	&-1 \cdot \sum_{ p l k q } \sum_{ \sigma \sigma' } B( l-p, p )  C_1( k+p, l-p, -p, -p+q )   c_{ k, \sigma }^\dagger  c_{ l, \sigma' }^\dagger  c_{ l-q, \sigma' } c_{ k+q, \sigma } 
\end{align*}
Inserting the definitions for $A$ and $B$ yields
\begin{align*}
	\partial_{\lambda} V(\lambda; k, l, q) \approx - \sum_{p} \bigg\{ &M_{\lambda}( k+q, p )  \sgn \left[ \epsilon( k+p+q )  - \epsilon( k+q ) +\omega( -p )  \right]C_2( k+p, l, -p, q ) \\
	+ &M_{\lambda}( l-q, p )  \sgn \left[ \epsilon( l+p-q )  - \epsilon( l-q ) +\omega( -p )  \right]C_2( k+p, l, -p, -p+q ) \\
	+ &M_{\lambda}^*( k, p )  \sgn \left[ \epsilon( k )  - \epsilon( k+p ) -\omega( -p )  \right]C_1( k, l, p, q ) \\
	+ &M_{\lambda}^*( l, -p )  \sgn \left[ \epsilon( l )  - \epsilon( l-p ) -\omega( p )  \right]C_1( k+p, l-p, -p, -p+q )  \bigg\}
\end{align*}
