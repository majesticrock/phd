...
From this, we learn \begin{align*}
	\partial_\lambda C_1(k,l,p,q) &= M_{p} e^{ - | \alpha (k,p) | \lambda } V^\mathrm{sym}( k, l, q )  - M_{p} e^{ - | \alpha (k+q,p) | \lambda } V^\mathrm{sym}( k+p, l, q ) . \end{align*}
Thus, we get \begin{align*}
	V(\lambda;k, l, q) \sgn \left[ \alpha (k, p) \right] M_{p} e^{ - | \alpha (k,p) | \lambda } - V(\lambda;k+p, l, q) \sgn \left[ \alpha (k+q, p) \right] M_{p} e^{ - | \alpha (k+q,p) | \lambda }. \end{align*}
...
\end{align*}
We plug in the results
\begin{align*}
	\partial_{\lambda} V(\lambda; k, l, q) \approx - \sum_{p} \bigg\{ &\sgn \left[ \alpha (k+q, p) \right] M_{p} e^{ - | \alpha (k+q,p) | \lambda } - V(\lambda;k+p, l, q) \sgn \left[ \alpha (k, p) \right] M_{p}^* e^{ - | \alpha (k,p) | \lambda } + V(\lambda;k, l, q) \sgn \left[ \alpha (k+q, p) \right] M_{p}^* e^{ - | \alpha (k+q,p) | \lambda }\\
	&\sgn \left[ \alpha (l-q, p) \right] M_{p} e^{ - | \alpha (l-q,p) | \lambda } - V(\lambda;k+p, l, -p+q) \sgn \left[ \alpha (k, p) \right] M_{p}^* e^{ - | \alpha (k,p) | \lambda } + V(\lambda;k, l, -p+q) \sgn \left[ \alpha (k-p+q, p) \right] M_{p}^* e^{ - | \alpha (k-p+q,p) | \lambda }\\
	&\sgn \left[ \alpha (k, p) \right] M_{p}^* e^{ - | \alpha (k,p) | \lambda } V(\lambda;k, l, q) \sgn \left[ \alpha (k, p) \right] M_{p} e^{ - | \alpha (k,p) | \lambda } - V(\lambda;k+p, l, q) \sgn \left[ \alpha (k+q, p) \right] M_{p} e^{ - | \alpha (k+q,p) | \lambda }\\
	&\sgn \left[ \alpha (l, -p) \right] M_{-p}^* e^{ - | \alpha (l,-p) | \lambda } V(\lambda;k+p, l-p, -p+q) \sgn \left[ \alpha (k+p, -p) \right] M_{-p} e^{ - | \alpha (k+p,-p) | \lambda } - V(\lambda;k, l-p, -p+q) \sgn \left[ \alpha (k+q, -p) \right] M_{-p} e^{ - | \alpha (k+q,-p) | \lambda }\\
	 \bigg\}
\end{align*}
