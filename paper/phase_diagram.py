...
axs[0].plot(micnas_d[0], micnas_d[1], "k-")
axs[0].plot(np.array([0, 2]), 0.25 * np.array([0, 2]), "k-")
axs[0].plot(np.array([-2, 0]),        np.array([0, 0]), "k-")

#Shade s-wave area
common_x = np.linspace(-2, 0, 100)
interpol = np.interp(common_x, micnas_d[0], micnas_d[1])
axs[0].fill_between(common_x, np.zeros(100), interpol, color="blue", alpha=.5, label="$s$-wave")

#Shade d-wave area
axs[0].fill_between(micnas_d[0], micnas_d[1], -2 * np.ones(len(micnas_d[0])), color="red", alpha=.5, label="$d_{x^2 - y^2}$-wave")

#Shade AFM area
common_x = np.linspace(0, 2, 100)
interpol = np.interp(common_x, micnas_d[0], micnas_d[1])
axs[0].fill_between(common_x, .25 * common_x, interpol, color="green", alpha=.5, label="AFM")

#Shade CDW area
axs[0].fill_between(np.array([-2, 0, 2]), np.array([0, 0, 0.5]), np.array([2, 2, 2]), color="orange", alpha=.5, label="CDW")
