...
Next, we decouple the the interaction terms.
We define 
\begin{equation}
    \label{eqn:operators}
    \begin{aligned}
...
\begin{equation}
    ...
    \langle \hat{O}_{\vk} \rangle = \langle \hat{O}_{\vk'} \rangle \eqqcolon \langle \hat{O}_{\gamma} \rangle\,,
\end{equation}
...
\begin{equation}
    \Delta_\text{SC} = \frac{U}{2N} \int_{-1}^{1} \dgamma \rho(\gamma) \langle f_{\gamma} \rangle\,.
\end{equation}
...
This method excells at dealing with the singularities in the DOS, 
requiring merely a few hundred function evaluations to achieve the desired accuracy.

This procedure yields the groundstate phase diagram at temperature $T=0$ shown in \autoref{fig:phase_diagram}.
...
\begin{equation}
    ...
    \mathcal{G}(z = \omega + i0^+) = \mN \frac{1}{-z \mN - \mM} \mN \coloneqq  -\mN \mathcal{R}(z) \mN\,,
\end{equation}
...
\end{equation}
Our basis is then $\mathcal{B}_{XP} \coloneqq  {X_i, P_i}$.
...
\begin{equation}
    \mathcal{R}(z) \vert_X = - \check{N}^{-1/2} \frac{1}{z^2 - \underbrace{\check{N}^{-1/2} \mathcal{K}_+ \check{N}^{-1/2}}_{\coloneqq  \check{M}}} \check{N}^{-1/2}\,,
\end{equation}
where $\vert_X$ denotes the block corresponding to the $X$-terms in the basis and $\check{N} \coloneqq  \mathcal{L} \mathcal{K}_-^{-1} \mathcal{L}^\dagger$.
...
\end{align}
Next, we define $\mathcal{L} \mathcal{K}_-^{-1} \mathcal{L}^\dagger \coloneqq  \check{N}$ for brevity.
