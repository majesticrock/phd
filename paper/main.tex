...
\begin{figure*}
    ...
    %The fits are linear, i.e., $y(V) = c \ln|V/t| + d$, and the parameters are printed on the individual panels. \textcolor{red}{ind möglicherweise auch gar nicht sinnvoll.}
...
Moving towards smaller $V$, the peaks move toward greater $\omega$, i.e., closer toward the two-particle continuum.
...
Therefore, we conclude that beyond these $V$, the superconducting phase is not the true groundstate of the system.
...
In recent research, there has been evidence for a phase seperated state for negative values of $V$,
...
Lastly, we investigate how the modes behave a the system transitions between the CDW and AFM phases.
...
The panels (a) show the peaks' positions unmodified. They essentially move linearly as $U$ is varied.
It is noteworthy, that the in the AFM phase, the peak in $\spectral{AFM}$ lies lower than the one in $\spectral{CDW}$.
...
The other peak, however, moves evercloser to the aforementioned continuum until they reach it.
At this point, the weight of said peak also vanishes, see panels (c).
This occurs around $U=4V \pm 0.4t$ for the square lattice and around $U=6V \pm 1t$ for the simple cubic lattice.

On the square lattice, the weights of the peak in $\spectral{AFM}$ in the AFM phase (the same applies for CDW) grows as the system moves away from the phase transition.
