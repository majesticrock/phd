...
So far we have discussed the static mean-field equations that essentially describe the static groundstate.
...
We can then express any time-depend operator from this set as
\begin{equation}
    a(t) = \sum_j c_j(t) A_j\,,
\end{equation}
...
For instance, a commuting a bilinear term with a quartic Hamiltonian yields, quartic terms. Repeating the commutation yields hexartic ones and so on.
...
To study the collective excitation, we investigate various retarded Green's functions 
\begin{equation}
    G_{AB}^\text{ret} (t) = - i \langle [A(t), B] \rangle \Theta(t)\,,
\end{equation}
where $\Theta(t)$ is the Heaviside function, and their Fourier transformed
\begin{equation}
...
Our operator set consists of operators of the type
\begin{equation}
...
Different indices are taken as additional set operators, e.g. $n_{\vk \up}$ and $n_{\vk \down}$ are taken into the set separately.
...
\end{equation}
Our set is then $\mathcal{B}_{XP} \coloneqq  {X_i, P_i}$.
...
\end{equation}
where $\vert_X$ denotes the block corresponding to the $X$-terms in the set and $\check{N} \coloneqq  \mathcal{L} \mathcal{K}_-^{-1} \mathcal{L}^\dagger$.
...
We embrace the matrix structure in \eqref{eqn:xp_set} and assume that all matrix entries are real.
Then, with $m|_X$ denoting the block of a matrix $m$ spanned by the $X$-operators in the set, we can calculate
\begin{align}
