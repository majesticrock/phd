...
\begin{figure*}
    ...
    For each plot, the parameters are set to $U=-2.5t$ and $N_\gamma = 3000$, while $V$ is varied to according to the text in the individual panels.
...
\begin{figure*}
    ...
    The left column shows the results for the square lattice while the right column shows the results for the simple cubic lattice.
    The shaded area marks the two-particle continuum.
    For each plot, the parameters are set to $V=1t$ and $N_\gamma = 3000$, while $U$ is varied to according to the text in the individual panels.
...
First and foremost, in the SC phase, see panel (a), there is a sharp peak located at $\omega=0$ in $\mathcal{A}_\text{Phase}$,
and another peak, only in $\spectral{Higgs}$, located at $\omega=2\Delta_\text{SC}$.
...
\end{equation}
with $r > 0$ and $u > 0$ if the system is outside of the superconducting phase. \textcolor{Zitieren!}
In this case, the only minimum is at $\Delta_\text{SC} = 0$. 
That means that both possible modes have to overcome the same energy to be excited in the first place. \textcolor{red}{Passt die Beschreibung so?}

At $V=0$, both the SC and the CDW phase coexist.
Here, $\spectral{Higgs}$ and $\spectral{CDW}$ are identical. Both have a peak at $\omega = 0$ and another one at $\omega = 2\Delta$.
...
For moderate sized values of $V>0$, i.e., in the CDW phase, see panel (c) at $V=0.1t$, the aforementioned spectral function has a peak at $\omega=2\Delta$.
...
\begin{figure*}
    ...
    In the upper panel, the pink markers additionally show the peak position devided by the gap $\omega_0 / \Delta_\text{CDW}$.
...
Deviding $\omega_0$ by the gap $\Delta_\text{CDW}$ results in a plateau for large $V$ while leaving the square root behavior at the beginning mostly unchanged.
