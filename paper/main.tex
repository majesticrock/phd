...
Collective excitations hold profound scientific interest as they unveil the intricate dynamics of correlated electron systems, offering key insights into the emergent properties of materials.
...
\textcolor{red}{Für den 3D Fall finde ich keine Phasendiagramme des extended Hubbard models.
Es gibt eines zum normalen Hubbard model \cite{Staudt00} und nen Haufen zum Bose-Hubbard-Model, aber das ist halt etwas anderes.}
...
In our study, we employ the extended Hubbard model because it encompasses various phases and thereby provides direct access to a rich spectrum of excitation spectra therein.
...
\begin{figure*}
    ...
    \caption{The upper panels (a) show plots of the positions $\omega_0$ of the peak in $\spectral{CDW}$ in the SC phase, 
    as well as the position relative to the lower edge of the continuum $\omega_-$.% and in units of $\Delta_\text{SC}$. 
    The lower panels (b) show their respective weights $w_0 = e^{b}$.
    The dashed lines represent the value of $\omega_0$ and $w_0$ computed for $U=-2.5t$ and $V=0$ while fixing $\Delta_\text{CDW}=0$.
    The left column shows the results for the square and the right column for the simple cubic lattice at $U=-2.5t$.
    ...
    \textcolor{red}{Der Fit im unteren Panel ist ein artanh(ln|V|), damit landet man bei sowas wie $(1-V^2)/(1+V^2)$.
...
Therefore, this phase may be chosen arbitrarily, and changing it requires no energy which ultimately allows a Goldstone mode to exist in the first place \cite{Goldstone1961,Anderson63}.
...
Both of them smoothly approach a finite value for $|V| \ll 1t$, marked by the dashed line.
These values are obtained by computing the peaks' position and weight for $V=0$ and setting $\Delta_\text{CDW}=0$, see \autoref{fig:resolvent_overview_V0} (a).

Moving towards smaller $V$, the peaks move towards greater $\omega$, i.e., closer to the two-particle continuum.
Additionally, the weights drop off, first with increasing speed and then slowing down.
They seem to converge towards some value, but we are unable to compute the Green's functions for additional, 
even smaller $V$ because the matrix $\mM$ is not nonnegative anymore after $V\approx -0.28t$ (square lattice) or $V\approx -0.34t$ (simple cubic lattice).
Note, that our proof for the nonnegativity is based on the system being in thermal equilibrium.
Therefore, we conclude that beyond these $V$, the superconducting phase is not the true ground state of the system.
...
In recent research, there has been evidence for a phase-separated state for negative values of $V$ on a square lattice,
which would explain our results here \cite{Linner23}.
...
This occurs around $U=4V \pm 0.4t$ for the square lattice and around $U=6V \pm 1.1t$ for the simple cubic lattice.
