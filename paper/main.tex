...
We proof this statement in the appendix \autoref{sec:positive_M}.
...
\end{align}
Later on, when we compute the spectral functions, we perform another two $\gamma$ integrations, for example, let us compute the first Lanczos coefficient for $A_\text{Higgs}$.
Then, the elements of the vectors mentioned in \eqref{eqn:green_derivation} are given by
\begin{equation}
    a_i = b_i = \begin{cases}
        \Delta \gamma & i \text{ corresponds to an } f\text{-type basis operator} \\
        0 & \text{otherwise}
    \end{cases}\,.
\end{equation}
