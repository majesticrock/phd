...
\begin{figure*}
    \centering
    \includegraphics[width=.98\textwidth]{plots/resolvent_overview.pdf}
    ...
    The panels (a), (b), and (c) show the spectral functions in the SC, CDW, and AFM phases, respectively.}
...
in various phases and afterward, we will describe the behaviour of the occurring modes as the system approaches a phase transition.
\newline
Firstly, all of the aforementioned operators are (anti-)Hermitian. Therefore, their commutator $[A^\dagger, A]$ vanishes.
Thus, abiding by the sum rule $\int_{-\infty}^\infty \mathcal{A}_{AB} (\omega) \mathrm{d}\omega = \langle [A^\dagger, A] \rangle = 0$ \cite{rickayzen80},
all spectral functions must be symmetrical about $\omega=0$. Therefore, we will only show the $\omega \geq 0$ in our plots.
\newline
Multiple spectral functions in the SC, CDW, and AFM phases are shown in \autoref{fig:resolvent_overview}.
