...
Obtaining an off-diagonal block requires linear combinations in the form of \textcolor{red}{Ich bin damit nicht glücklich, da $\mathcal{G}$ ja eigentlich nicht hermitisch ist (wegen dem $z = \omega + i0^+$).
Das ist für die Ergebnisse völlig egal, da ich eh nur diagonale Blöcke betrachte, aber nunja...}
\begin{subequations}
    \begin{align}
        4 \Re [ \vec{a}^\dagger \mathcal{G} \vec{b} ] &= (\vec{a} + \vec{b})^\dagger \mathcal{G} (\vec{a} + \vec{b}) - (\vec{a} - \vec{b})^\dagger \mathcal{G} (\vec{a} - \vec{b}) \nonumber \\
            &= 2 \vec{a}^\dagger \mathcal{G} \vec{b} + 2 \vec{b}^\dagger \mathcal{G} \vec{a}\,, \\
        4i \Im [ \vec{a}^\dagger \mathcal{G} \vec{b} ] &= (\vec{a} + i \vec{b})^\dagger \mathcal{G} (\vec{a} + i \vec{b}) - (\vec{a} - i \vec{b})^\dagger \mathcal{G} (\vec{a} - i \vec{b}) \nonumber \\
            &= 2i \vec{a}^\dagger \mathcal{G} \vec{b} - 2i \vec{b}^\dagger \mathcal{G} \vec{a}\,.
    \end{align}
\end{subequations}
...
To begin, let us describe the the spectral function with respect to the operators above in the various phases.
...
We plot it in close proximity to the peaks with a double logarithmic scale and fit the result linearly $y = ax + b$ to obtain its power-law behaviour.
...
Next, we will investigate the Higgs peak, this time around, turning towards the spectral function itself. 
...
Again, the both SC-related spectral functions are identical in both the CDW and AFM phase.
...
In the CDW phase, this behaviour is reversed, i.e., the CDW peak lies lower.
...
\end{equation}
and thereby mimicking the continuous $\delta$ distribution for a discrete description.
Note, that if one takes the limit $\Delta \gamma \to 0$ there $\delta$ distribution is exactly reproduced.

Numerically, we can compute the matrix elements like this
\begin{align}
    (A_i | A_j) &= \frac{(\Delta \gamma)^2}{N} \sum_{\vk \vl} h(\gamma_i - \hat{\gamma}(\vk)) h(\gamma_j - \hat{\gamma}(\vl)) (A_{\vk} | A_{\vl})\,.
\end{align}
Here, all expression are finite numbers and do not require further treatment.
