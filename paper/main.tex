...
\textcolor{red}{The amplitude mode, on the other hand, is located at the lower edge of the quasiparticle continuum as it represents the energy required to form or break a Cooper pair \cite{Schmid1975,Varma02,Cea14,Measson14,Tsuji15,Krull16,Schwarz20}}
...
\begin{figure}
    ...
    In accordance with literature, we expect to find a phase-separated state \cite{Linner23}.
...
\textcolor{red}{Additionally, we will include 
\begin{equation}
...
This happens if the paremeters are chosen from the purple shaded area in \autoref{fig:phase_diagram}.
...
However, we did not perfom any computations in that region, hence this does not pose an issue to our analysis.}
...
\textcolor{red}{That means, if we index each operator in our set $A_i$, the matrix element $(i,j)$ of $\mathcal{G}$ is the Green's function
\begin{align}
    \mathcal{G}_{ij}(z = \omega +i0^+) &=  G_{A_i A_j^\dagger} (z) \nonumber \\
        &= -i \int_0^{\infty} \langle [A_i(t), A_j^\dagger(0)] \rangle e^{izt} \mathrm{d}t\,.
\end{align}}
...
that \textcolor{red}{$\vec{A} \cdot \vec{a}$} is equal to the desired operator. 
