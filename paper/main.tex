...
\begin{equation}
    ...
    \begin{aligned}
        ...
        \Rightarrow \sum_j \underbrace{(A_i | A_j)}_{:=\mN_{ij}} \ddt c_j(t) &= i \sum_j \underbrace{(A_i | [H, A_j])}_{:=\mM_{ij}} c_j(t) \\
        \Rightarrow \mN \ddt \vec{c}(t) &= i \mM \vec{c}(t)\,.
    \end{aligned}
\end{equation}

The matrices $\mN$ and $\mM$ contain all the dynamics and energetic properties of the system but have the significant advantage of being simple matrices rather than operators acting on an enormous Hilbert space.
\newline
...
Nevertheless, we will compute $\mM$ using the full Hubbard Hamiltonian \eqref{eqn:full_hamiltonian} enabling us to capture collective behaviour rather than the one particle dynamics given by the mean-field approximation.
\newline
...
\begin{equation}
    ...
    \mathcal{G}(z = \omega + i0^+) = \mN \frac{1}{-z \mN - \mM} \mN := -\mN \mathcal{R}(z) \mN\,,
\end{equation}
...
\begin{figure}
    ...
    \begin{subfigure}{0.48\textwidth}
        \includegraphics[width=\textwidth]{plots/phase_peak.pdf}
        \caption{Phase Green's function}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\textwidth}
        \includegraphics[width=\textwidth]{plots/cdw_peak_in_sc.pdf}
...
\begin{figure*}
    ...
    The left column shows the results for the square lattice at $U=-2$ and the right column for the simple cubic lattice at $U=-2.5$.
    The fits are linear, i.e. $y(V) = c \ln(V/t) + d$ and the parameters are printed on the individual panals.}
...
In this appendix section, we will derive the relation between the matrices $\mM, \mN$ from \eqref{eqn:heisenberg} and the Green's functions of the system.
For that, consider their definition

\begin{align}
    G_{AB^\dagger}^\text{ret} (t) &= - i \langle [ A(t), B^\dagger(0) ] \rangle \theta(t) \nonumber \\ 
    \Rightarrow G^\text{ret} (z = \omega + i0^+) &= - i \int_0^\infty \exp(izt) \langle [ A(t), B^\dagger(0) ] \rangle \mathrm{d}t \nonumber
\end{align}

Since $A$ and $B$ are included in the operator basis, we can rewrite them as dot-products of the vector
\begin{equation}
    \vec{A} := \left( A_1, A_2, \dots, A_N \right)\,, \quad
    \vec{A}^\dagger := \begin{pmatrix}
        A_1^\dagger \\ A_2^\dagger \\ \vdots \\ A_N^\dagger
    \end{pmatrix}\,,
\end{equation}

and some other vector with complex-valued vector $\vec{a}$ which has its coefficients set in such a way,
...
\begin{align}
    G^\text{ret} (z) &= i \int_0^\infty \exp(izt) \langle [ (\vec{A} \vec{b}(0))^\dagger, \vec{A} \vec{a}(t) ] \rangle\ \mathrm{d}t \nonumber \\
...
Normally, one would desire to compute

\begin{align}
    \mathcal{R}(z) = \mN^{-1/2} \frac{1}{z +\mN^{-1/2} \mM \mN^{-1/2}} \mN^{-1/2}\,,
\end{align}

and then utilize a Lanczos algorithm to obtain a tridiagonal form of $\mN^{-1/2} \mM \mN^{-1/2}$.
However, $\mN$ is not positive in our case due to the nature of the commutator, which means that $\mN$ cannot have a real square root.
One could also fathom computing

\begin{align}
    \mathcal{R}(z) = \frac{1}{z} \mM^{-1/2} \frac{1}{\mM^{-1/2}\mathcal{N}\mM^{-1/2} + \frac{1}{z} } \mM^{-1/2}\,,
\end{align}

which does in fact work, if $\mM$ is positive, however resolving feature that lie near $z=0$ are difficult to resolve.
...
\begin{align}
    \vec{x}^\dagger \mM \vec{x} &= \sum_{ij} x_i^* x_j \mM_{ij} \nonumber \\
        ...
        &= \langle [B^\dagger, [H, B]] \rangle\,,\quad B := \sum_j x_j A_j\,.
\end{align}
...
\begin{widetext}
    \begin{align}
        \langle [B^\dagger, [H, B]] \rangle &= \frac{1}{Z} \Trace [\exp(-\beta H) [B^\dagger, [H, B]]] \nonumber \\
            ...
            &= \frac{1}{Z} \sum_{ij} e^{-\beta \epsilon_j} (\epsilon_i - \epsilon_j) \underbrace{ \left[ \bra{\psi_j} B^\dagger \ket{\psi_i} \bra{\psi_i} B \ket{\psi_j} + \bra{\psi_j} B \ket{\psi_i} \bra{\psi_i} B^\dagger \ket{\psi_j} \right]}_{:=C_{ij} = C_{ji} \geq 0} \nonumber \\
...
We embrace the matrix structure in \eqref{eqn:xp_basis} and assume that all matrix entries are real.
Then, with $m|_X$ denoting the block of a matrix $m$ spanned by the $X$-operators in the basis, we can calculate

\begin{align}
    r(z) &:= \mathcal{R}(z)|_X = \left. \frac{1}{\mM + z \mN} \right\vert_X \nonumber \\
...
Next, we define $\mathcal{L} \mathcal{K}_-^{-1} \mathcal{L}^\dagger := \check{N}$ for brevity.
...
The inverse in the following can be replaced by the pseudo-inverse in case $\check{N}$ has a singular part.
We obtain
...
The matrix $\check{M} := \check{N}^{-1/2} \mathcal{K}_+ \check{N}^{-1/2}$ is positive semidefinite and Hermitian by the previous arguments.
Hence, we can tridiagonalize it using a standard Lanczos algorithm in order to obtain a continued fraction expansion.
