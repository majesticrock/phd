...
\begin{figure*}
    ...
    The panels show the spectral functions in the (a) pure SC phase, (b) SC/CDW-coexistence, and (c) and (d) pure CDW phase, respectively.}
...
First and foremost, in the SC phase, see panel (a), there is a sharp peak located at $\omega=0$ in $\mathcal{A}_\text{Phase}$ and a singularity in $\spectral{Higgs}$ located at $\omega=2\Delta_\text{SC}$.
...
This analysis provides that the real part of $\greens{Phase}$ behaves as $1/\omega^2$ indicating that the peak in $\spectral{Phase}$ is the derivative of a $\delta$ distribution.
...
\textcolor{red}{Ich spekuliere mal...}
...
These have the frequency $\Omega = 2 \Delta_\text{SC}$ and fell of as $1/\sqrt{t}$.
...
In the CDW phase, see panels (c) and (d), both of $\spectral{Higgs}$ and $\spectral{Phase}$ become identical and form a $\delta$ peak below the two-particle continuum.
...
Since the degeneracy of both phases is exact due to the SO(4) symmetry \cite{yang90}, there should not be any difference between $\spectral{Higgs}$ and $\spectral{CDW}$ as both spectral functions capture the amplitude fluctuations of their respective order parameters.
This is precisely what can be seen in panel (b) of \autoref{fig:resolvent_overview_SC}.
Both spectral functions have a peak at $\omega = 0$ and a singularity at the lower edge of the two-particle continuum $\omega = 2\Delta_\text{tot}$, with $\Delta_\text{tot} \coloneqq \sqrt{|\Delta_\text{CDW}|^2 + |\Delta_\text{SC}|^2}$.
...
Thus, we associate the peak at $\omega=0$ with this shifting of order parameters.
The singularity at the lower edge of the two-particle continuum can be interpreted in a similar manner as the Higgs mode in a pure superconductor.
...
For moderate-sized values of $V>0$, i.e., in the CDW phase, see panel (c) at $V=0.1t$, the aforementioned spectral function has a singularity at $\omega=2\Delta_\text{CDW}$.
This mode becomes hard and moves outside the two-particle continuum as $V$ is increased, see panel (d) at $V=0.5t$.
\textcolor{red}{Ich muss mal suchen, ob sowas schonmal beobachtet wurde.}
...
The plots for the phase peak and the CDW peak in the SC phase can be found in appendix \autoref{sec:fit_greens_functions}.
...
First and foremost, we investigate the peak in $\mathcal{A}_\text{Phase}(\omega)$ and $\mathcal{A}_\text{Higgs}(\omega)$ in the CDW phase.
...
For $V \ll 1$, the position grows with the square root of $V$. This behavior then transitions to a purely linear growth for large $V$.
...
\begin{figure}
    ...
    \caption{Log-log plot of the real part of $\spectral{Phase}$ in the SC phase at $U=-2.5t$ and $V=-0.1t$ as well as a linear fit to it.
    The upper (lower) panel shows the results for the square (simple cubic) lattice.
    The function behaves as $1/\omega^2$ indicating that the peak in the spectral function is the derivative of a $\delta$ distribution.}
    \label{fig:phase_peak}
\end{figure}
...
\begin{figure*}
    ...
    \caption{Log-log plot of the real part of various Green's functions, the labels indicate its type and the text in the panels indicate the phase.
        ...
        The power-law exponent is $a=-1$ in cases, indicating that these peaks are $\delta$ distributions.}
        \label{fig:peak_fits}
\end{figure*}
...
Firstly, the real part of $\greens{}{Phase}$ is shown in \autoref{fig:phase_peak}.
...
The peaks of $\spectral{AFM}$ and $\spectral{CDW}$ close the the phase transition are identical when swapped, i.e., $\mathcal{A}_\text{AFM} (\omega)$ in the CDW phase is the same as $\mathcal{A}_\text{CDW} (\omega)$ in the AFM phase.
...
Note, that the sum going up to $2N_\gamma$ rather than $N_\gamma$ merely arises from the fact, that we need to integrate over both $f_\gamma$ and $f_\gamma^\dagger$ to obtain $\greens{Higgs}$.
