...
for i in range(2):
    ...
    for name_suffix in name_suffices:
        ...
        # V = 0, Delta_CDW = 0
        peak_pos_value, peak_weight = rp.analyze_peak(f"{folders[i]}T={Ts[0]}/U={4.0 if i == 0 else 6.0}/V=1.0", f"higgs_{name_suffix}")
        cont_edges = cf.continuum_edges(f"{folders[i]}T={Ts[0]}/U={4.0 if i == 0 else 6.0}/V=1.0", f"higgs_{name_suffix}", True)
        peak_pos_value = cont_edges[0] - peak_pos_value
        #axs[0][i].axhline((peak_pos_value), color="black", linestyle="--")
        #axs[1][i].axhline(np.exp(peak_weight), color="black", linestyle="--")
        
        plotters[0][i].plot(np.log(Us), np.log(np.abs(peak_pos_value - peak_positions_to_cont)), label=f"Data - $\\mathcal{{A}}_\\mathrm{{{name_suffix}}} (\\omega)$")
        plotters[1][i].plot(np.log(Us), ((np.exp(peak_weight) - np.exp(weights))))
