...
Us_cube = np.array([0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1.0, 1.1])
Vs = np.array([1.0])
...
for i in range(2):
    ...
    for name_suffix in name_suffices:
        ...
        plotters[0][i].plot(np.log(Us), (peak_positions_to_cont), label=f"Data - $\\mathcal{{A}}_\\mathrm{{{name_suffix}}} (\\omega)$")
        plotters[1][i].plot(np.log(Us), np.exp(weights))
