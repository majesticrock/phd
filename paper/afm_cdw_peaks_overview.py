...
# Hinzufuegen: 0.5, 0.45, 0.4, 0.35, 0.25
Vs = np.array([1.0])
...
peak_positions_to_cont = np.zeros(len(Us))
nrows=3
...
for i in range(2):
    ...
    for name_suffix in name_suffices:
        ...
        plotters[0][i].plot(u_data, peak_positions, label=f"Data - {name_suffix}")
        plotters[1][i].plot(u_data, peak_positions_to_cont, label=f"Data - {name_suffix}")
        plotters[2][i].plot(u_data, np.exp(weights), label=f"Data - {name_suffix}")
