...
for idx, (plot_data, label, screening) in enumerate(zip(dfs, ["Approx. $g(k,k')$", "Full $g(k,k')$", r"$\lambda=1$", r"$\lambda=10^{-4}$"], [None, None, 1, 1e-4])):
    ...
    # Confidence interval (essentially it's a perfect fit)
    #u_opt = correlated_values(popt, pcov)
    #y_fit_unc = fit_func(g_lin, *u_opt)
    #y_fit_nominal = unp.nominal_values(y_fit_unc)
    #y_fit_std = unp.std_devs(y_fit_unc)
    #
    #plt.fill_between(g_lin, y_fit_nominal - 1.96 * y_fit_std, y_fit_nominal + 1.96 * y_fit_std,
    #                color=f"C{idx}", alpha=0.3)#, label="95% CI"
